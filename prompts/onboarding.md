...
> 4. From my answers, create the first structure — directories with `INDEX.md` files — and seed it with dense starter notes: entity pages for the key people and organizations, the current state of play, open questions.
> 5. Commit from `<PATH>` with `git add -A . && git commit`, then tell me what you stored and where. If git identity is not configured, tell me exactly what remains uncommitted.
>
