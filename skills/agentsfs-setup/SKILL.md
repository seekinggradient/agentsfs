...
- Normal path: from the project directory, run `afs setup` (or `afs setup <path>`). This creates or reuses the personal agentsfs and connects the current project to it.
- Existing agentsfs, new project: from the project directory, run `afs connect <path>` — it appends a connection block to the project's AGENTS.md/CLAUDE.md (offers to create one if absent).
- Lower-level create-only path: `afs init <path>` creates an agentsfs exactly where requested. If `<path>` is inside a git repo, use `afs init ./agentsfs --shared` only when the user explicitly wants team-shared memory committed with the code.
...
4. Commit from the agentsfs root: `git add -A . && git commit`.
