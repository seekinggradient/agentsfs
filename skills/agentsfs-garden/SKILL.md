...
3. **Then densify:** merge stubs and overlapping notes into the better file; delete what you merged. Use `afs rename <old> <new>` when a better name helps — it rewrites all links in one pass. A **collection** (a directory marked `agentsfs_role: collection` — a diary, daily notes, attachments) is hands-off: never densify, describe, reorganize, or delete inside it; updating the collection's own INDEX `description:` is the only change allowed.
