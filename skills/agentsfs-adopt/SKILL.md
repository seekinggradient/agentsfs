...
2. **Mark the reserved roles.** The session journal and scratch space are declared by a marker (`agentsfs_role: journal` / `agentsfs_role: scratch` in a directory's `INDEX.md`), not by name. **Never claim an existing personal directory as the journal** — its agent-owned lifecycle does not apply to a diary. If the user wants a journal or scratch, create or designate one *with them* (the default `agent-journal/` is safe).
