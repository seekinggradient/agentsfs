...
- **Standard filesystem is the interface; the toolkit augments, never wraps.** Agents use their native fluency — `ls`, `grep`, `cat`, plain reads and writes — directly on the files. There is no `afs ls`. CLI commands exist only for what plain tools can't do cheaply: cross-instance discovery/status, aggregated views (tree with descriptions), derived structure (backlinks, search), cross-file rewrites (rename), and health (doctor). `afs status` discovers from filesystem markers on demand rather than introducing a stale central registry; its default scan is local/read-only and contract mutation remains per-instance. A wrapper CLI is rejected because it would make the toolkit load-bearing (violating the contract/toolkit split) and tax the training distribution (Principle 5).
...
**Deliberately not built:** any harness hook or transcript integration (files are the only carrier; at most a single reference Claude Code hook may exist someday as documentation, never a supported surface); the **scheduled/hosted gardener** — the consumer that would make the loop automatic — which remains Hub/Eve product work (see [hub-execution-plan.md](hub-execution-plan.md)); automatic journal capture from Hub-backed Eve thread archives; and a generalized drop-anything `inbox/` (parked — one concept per reserved name). The old idea of waking a per-user Sprite and reading an `agentsfs-chat` transcript is superseded by the Vercel Eve + Hub-thread architecture.
...
### Layer 5b — collections: adopting existing vaults — AGREED (2026-07-08)

**The problem.** Adopting an existing Obsidian vault (a blessed use case — the owner's real vault was the first) leaves hundreds of permanent doctor findings: personal chronology (`Journal/`, `Daily/`), media (`Attachments/`, `Excalidraw/`) will never carry per-file `description:` frontmatter — and never *should*, because the human keeps writing frontmatter-less notes daily, so an adopted vault can never stay doctor-clean under per-file rules. The measured breakdown on the owner's vault: 706 findings; the active area (`Projects/`, 275) is worth real annotation, the personal/media dirs are not. Three non-designs were rejected: permanent noise (teaches everyone to ignore doctor — kills the health system), a perpetual annotation treadmill (agents forever adding frontmatter to a private diary — fails the brother test), and prose-only exemptions tools can't honor (the divergence 5a just eliminated).

**The design (contract 0.5.0): a third role value, not a new mechanism.** `agentsfs_role: collection` in a directory's INDEX.md declares a body of like items described *collectively* by that INDEX: beneath it, doctor requires no per-file descriptions, no per-directory INDEXes, and reports no stub/orphan/link findings — while search, tree, and indexing remain fully on (nothing is deletable; this is durable content at coarser description granularity). Collections are repeatable (unlike the journal/scratch singletons). This extends the contract's *existing* second description path — "files that can't hold frontmatter are described in their directory's INDEX.md" — to files that shouldn't have to, and keeps the invariant intact at declared granularity: the tree always explains itself. Guardrail: **roles are a closed contract-defined set** — journal, scratch, collection; no user-definable roles, no per-file exemptions, no doctor config language, ever. A fourth role must earn its place through a real incident, as each of these did.

**Alongside:** the walker ignores dot-directories (`.obsidian/`, `.trash/` — machine territory by convention; doctor was flagging Obsidian's own config), and the adoption flow ships as product: `prompts/adopting.md` + an `agentsfs-adopt` skill — mark roles, declare collections for personal chronology/media, genuinely annotate active knowledge areas, fix dead links there, additive-only (bodies never touched), reviewable commits, doctor as the progress meter. The gardener learns to keep hands off collection contents.

**Gate 5b:** the owner's real vault reaches `doctor: healthy` via the adoption flow — collections declared for `Journal/`/`Daily/`/`Attachments/`/`Excalidraw/`, active areas annotated for real — and *stays* healthy after a new frontmatter-less note is added to a collection.

### Layer 5c — the root description lives in a root INDEX.md — AGREED (2026-07-17)

**The defect.** The root described itself through its `AGENTS.md` frontmatter `description:` — the value `afs tree`, `afs status`, and the Hub instance list all surface as the label for the whole workspace. But `AGENTS.md` is the *contract*, and its stock description is boilerplate ("Self-describing root of this agentsfs. Read this first…"); the contract never told an agent to make it instance-specific, and `afs contract upgrade` rewrites `AGENTS.md` wholesale, so a real description put there would be clobbered on the next upgrade anyway. Result: every real instance kept the boilerplate forever, and every surface that lists instances showed the same meaningless label — the one description that should be the most instance-specific was the most generic. (Confirmed on the owner's fleet: all 11 real workspaces showed the contract boilerplate; 8 had no root `INDEX.md` at all.)

**The design (contract 0.7.0): give the root its own INDEX.md, the way every other directory has one.** The per-instance description belongs to the *directory*, not the contract file — so the root becomes an ordinary directory with an `INDEX.md`, and `AGENTS.md` goes back to describing only the contract.

1. **The root has an `INDEX.md`.** The template ships `template/INDEX.md` whose `description:` is `REPLACE ME: one or two sentences describing what THIS workspace is about and what lives in it.` — a demand, not a value. Because it lives outside the contract-managed `AGENTS.md`, `contract upgrade` never overwrites it once an agent fills it in.
2. **The contract instructs it.** Rule 2 (INDEX maintenance) gains a sentence: the root is a directory too; its `INDEX.md` `description:` is this workspace's one-line summary — what it is about, not the generic fact that it is an agentsfs — and it lives there rather than in `AGENTS.md` so upgrades never overwrite it. The onboarding, adopting, and gardening prompt packs carry the same instruction so seeding, adoption, and maintenance all set it.
3. **Description resolution prefers it.** `DirDescription(".")` and the Hub's `repoFilesMeta` (which feeds the dashboard, the repo header, and the agent API `GET /repos` JSON) prefer the root `INDEX.md` description, falling back to `AGENTS.md` then `README.md` for older instances. A template placeholder or the pre-0.7.0 boilerplate is treated as *no* description in the Hub path, so a listing surfaces a real per-workspace summary or nothing — never the shared boilerplate.
4. **Doctor nags until it is real, and creates what is missing.** A new `root-index` warning fires when there is no root `INDEX.md` (the 8 legacy workspaces); a `root-description` warning fires when one exists but its description is missing, the current placeholder, or the legacy boilerplate — detecting *both* template phrasings (`REPLACE ME` prefix; the "Self-describing root of this agentsfs" signature). Both are `warn`, not `error`: a fresh instance is legitimately unfilled for a moment. Governed, never silent: `afs contract upgrade` *creates* a missing root `INDEX.md` (with the placeholder) as part of the reviewable upgrade diff — it never writes the description itself, which is the agent's judgment call about the instance's content.

**Gate 5c:** a fresh `afs init` lays down a root `INDEX.md` and is doctor-clean except for the single `root-description` nudge; setting a real description clears it; a pre-0.7.0 instance with no root `INDEX.md` reports `root-index`, and `afs contract upgrade` creates one (placeholder) without touching `AGENTS.md`'s neighbors; the Hub shows the root `INDEX.md` description and never the contract boilerplate; the fixture carries a real root `INDEX.md` description that surfaces in `afs tree` and the Hub, and the contract change adds no new findings to it.
...
3. **Hub scheduled agents (the loop's consumer)** — direction agreed 2026-07-06, implementation still pending. The original wake-a-Sprite proposal is superseded: production uses Vercel-hosted Eve, Hub-backed thread records, and the Hub agent API. A future gardener/research scheduler should use that architecture or a dedicated permission-scoped worker. Per-repo collaborators have already shipped and are no longer part of this decision queue.
...
- **2026-07-06 — The loop's division of labor agreed.** Continual learning splits edge/center: edge harnesses do the universal cheap thing (append a journal entry — any tool that can write a file complies); the center does the hard thing (scheduled consolidation on the Hub, later skill-promotion). Per-harness hooks/transcript integrations rejected as a supported surface. **Historical implementation note:** this originally assumed `agentsfs-chat` transcripts in a Sprite; current production stores Eve thread records/events through the Hub, so deterministic hosted capture must use that source instead.
- **2026-07-08 — Layer 5b (collections) agreed.** Owner raised the fragmentation concern explicitly; resolved by making adopted-vault support a third *value* of the existing role marker (`agentsfs_role: collection`) rather than a new mechanism. Doctor treats a collection's subtree as collectively described (no per-file findings); search/tree/index unchanged. Roles stay a closed set. Dot-directories become machine territory in the walker. Adoption ships as a packaged flow (`prompts/adopting.md`, `agentsfs-adopt` skill). Contract bumps 0.4.0 → 0.5.0.
...
- **2026-07-17 — Layer 5c (root description) agreed.** Every workspace's Hub/`afs status`/tree label was the same contract boilerplate, because the root description was read from the contract-managed `AGENTS.md` (which upgrade rewrites) and never made instance-specific. Fixed by giving the root its own `INDEX.md` — the per-workspace description lives there, outside the contract, so upgrades never clobber it — with a `REPLACE ME` placeholder the contract (rule 2) and the onboarding/adopting/gardening prompts tell agents to replace. Description resolution (`DirDescription`, the Hub's `repoFilesMeta` feeding the dashboard, repo header, and agent API `GET /repos`) prefers the root `INDEX.md`, falls back to `AGENTS.md`/`README.md`, and treats a placeholder or the legacy boilerplate as no description so listings never show boilerplate. Doctor gains `root-index` (missing root INDEX.md — the 8 legacy workspaces) and `root-description` (empty/placeholder/legacy) warnings; `afs contract upgrade` creates a missing root `INDEX.md` as a reviewable diff, never writing the description itself. Contract bumps 0.6.0 → 0.7.0.
- **2026-07-24 — `afs skills` ships the skill pack inside the binary.** The skills (setup, remember, adopt, garden) now embed in the afs binary; `afs skills [list]` lists them and materializes them under the afs config dir (`<config>/agentsfs/skills/`), refreshed on every run, so any install can copy them without a repo checkout. Deliberately list-only and harness-neutral: there is no `install` subcommand, and afs never writes into `~/.claude/` or any harness directory — Claude Code is one example destination, not a target. The scheduled/hosted gardener that would *run* a skill on the owner's behalf remains separate Hub work.
