...
The **+** beside Knowledge opens the same naming dialog. Right-click a folder and choose **New file here**, or use the **+** on its row, to create inside it without typing the folder path. This works in both the workspace file list and the reading sidebar. Keyboard users can focus a folder and press Shift+F10; Escape dismisses the menu or dialog.
...
HTML previews support relative `<img src="...">` links to SVG and raster images in the same workspace, including private and LFS-stored images. Hub embeds these into the preview without changing the stored HTML. This applies to documents up to 2 MiB, images up to 2 MiB each, at most 128 distinct image lookups, and a 16 MiB embedding budget. Missing or over-budget images retain their original links. CSS image URLs, `srcset`, and other relative resources are not rewritten. File-scoped public share links retain their existing behavior and do not gain access to other workspace files.

For HTML files, **Download → Standalone HTML** saves a single `.standalone.html` file with those workspace images embedded, including SVG and LFS images. Inline styles and scripts are preserved; **Original file** still returns the exact stored source. Standalone download refuses missing, unsupported, or over-budget workspace image references instead of silently omitting them. External URLs, fonts, stylesheets, CSS image URLs, and `srcset` are not bundled and may still require a connection or separate files.
...
Choose **Name version** to review the changes and optionally publish a version immediately with your own description. Automatic versions use a description such as `Edit note.md`, authored by you. Cmd/Ctrl+S requests an immediate autosave. **Download draft** exports the complete Markdown. While offline, the editor keeps a browser recovery copy and retries when the connection returns; it warns before leaving if the latest writing has not reached the Hub.

If another writer changes a different file, both changes are preserved. If the same note changes, the editor preserves your draft and offers the latest version beside it. Combine the changes and acknowledge your review to resume automatic saving. Retrying an acknowledged save does not create a duplicate commit. Draft session history is retained separately for recovery, and shared commits are never rewritten. The editor enforces the same collaborator permissions and projection upgrade policy as other Hub writes.
