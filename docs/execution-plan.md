...
**The defect.** The root described itself through its `AGENTS.md` frontmatter `description:` — the value `afs tree`, `afs status`, and the Hub instance list all surface as the label for the whole knowledge base. But `AGENTS.md` is the *contract*, and its stock description is boilerplate ("Self-describing root of this agentsfs. Read this first…"); the contract never told an agent to make it instance-specific, and `afs contract upgrade` rewrites `AGENTS.md` wholesale, so a real description put there would be clobbered on the next upgrade anyway. Result: every real instance kept the boilerplate forever, and every surface that lists instances showed the same meaningless label — the one description that should be the most instance-specific was the most generic. (Confirmed on the owner's fleet: all 11 real KBs showed the contract boilerplate; 8 had no root `INDEX.md` at all.)
...
- **2026-07-17 — Layer 5c (root description) agreed.** Every KB's Hub/`afs status`/tree label was the same contract boilerplate, because the root description was read from the contract-managed `AGENTS.md` (which upgrade rewrites) and never made instance-specific. Fixed by giving the root its own `INDEX.md` — the per-KB description lives there, outside the contract, so upgrades never clobber it — with a `REPLACE ME` placeholder the contract (rule 2) and the onboarding/adopting/gardening prompts tell agents to replace. Description resolution (`DirDescription`, the Hub's `repoFilesMeta` feeding the dashboard, repo header, and agent API `GET /repos`) prefers the root `INDEX.md`, falls back to `AGENTS.md`/`README.md`, and treats a placeholder or the legacy boilerplate as no description so listings never show boilerplate. Doctor gains `root-index` (missing root INDEX.md — the 8 legacy KBs) and `root-description` (empty/placeholder/legacy) warnings; `afs contract upgrade` creates a missing root `INDEX.md` as a reviewable diff, never writing the description itself. Contract bumps 0.6.0 → 0.7.0.
