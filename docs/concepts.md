...
A **reserved role** is a job a directory can be assigned — currently **journal**, **scratch**, **collection**, and **backlog** — resolved by a marker, not by name. All four are declared in the directory's own `INDEX.md` frontmatter (`agentsfs_role: <role>`, contract 0.4.0; the backlog joined them in 0.11.0, after one version as a page-level role, and has no name fallback at all). The default names (`agent-journal/`, `agent-scratch/`, `backlog/`) are just what `afs init` happens to lay down; any directory can be marked for its role, which is how an adopted, pre-existing folder gets one without being renamed. A 0.10.0 instance whose backlog is still a marked *page* keeps working read-only — `afs roles` reports it with `backlog_legacy: true`, doctor raises `backlog-page-role-legacy`, and `afs contract upgrade` moves it into the directory.

Journal, scratch, and backlog are **singular**: exactly one directory may hold each role. A second directory marked for the same role is a `duplicate-role` finding — severity `error`, and the *only* finding code that makes `afs doctor` exit non-zero. Collection is **repeatable**: any number of directories may be marked `agentsfs_role: collection`, each one a body of like items (a diary, daily notes, attachments) described collectively by its own `INDEX.md` rather than file-by-file. Doctor exempts everything strictly beneath a collection directory from per-entry findings (missing description, dead links, stubs) — the collection stays fully indexed and durable, just not annotated file-by-file.
...
**The rule that matters most:** call `afs roles [--json]` to find where a role actually lives. Never hardcode `agent-journal/` or `agent-scratch/` in a tool or a doc — the contract has changed these default names once already (0.4.0) and can again. The JSON carries `journal`, `scratch`, `collections`, and three backlog fields: `backlog` (the directory, or the marked page on a legacy instance), `backlog_spine` (always the task page — `<backlog>/INDEX.md`, or the legacy page), and `backlog_legacy` (true for the retired page shape). A consumer that wants tasks reads `backlog_spine`; one that wants the tickets and archive reads `backlog` after checking `backlog_legacy`. `backlog` changed meaning in 0.11.0 — a deliberate break, which is why the spine got its own field.
...
The **backlog** is the directory marked `agentsfs_role: backlog` (`backlog/` by default) — the instance's prospective memory: pending work, discovered tasks, and parked ideas. Its `INDEX.md` is both the directory's description and the **spine**: checkbox lists under priority bands (`Now`, `Next`, `Later`, `Someday`, `Done`), and the sole priority authority. Around the spine sit three things, none of them created by default: **ticket files** — ordinary notes beside the spine, earned when a task accumulates real state, whose body is current truth (rule 5) and whose `## Log` section is append-only dated events; **`archive/`**, a collection holding closed tickets stamped `closed: YYYY-MM-DD` and per-year rollups of closed one-liners; and **sub-backlogs**, subdirectories with their own spine (no marker of their own) whose tasks are ready only while a root task delegates to them.

The write path is ordinary markdown editing; the read path is derived views — `afs tasks` computes the ready-work queue (open, in a ready band, no open children, no active blocker) and, given a search root, groups it per instance, `afs prime` puts the top of it in front of a starting session, `afs task claim|done` flips one checkbox and commits, doctor checks the whole shape (duplicate slugs, dangling references, inconsistent parents, orphaned and unarchived tickets, undelegated sub-backlogs), and the Hub renders it with status controls and per-band progress. The grammar lives as a short legend on the spine itself and as rule 13 of the contract: `[ ]`/`[/]`/`[x]`/`[-]` markers, nesting as decomposition, trailing `^slug` anchors referenced as `[[backlog/INDEX#^slug]]`, `— blocked by [[#^slug]]` annotations that lift automatically when their targets close, and `— blocked by owner: <question>` for a decision only the owner can make (the agent parks it and pulls the next item; it never lifts on its own).

**Not:** a work-dispatch system. There is no claiming beyond the `[/]` marker, no leases, no orchestration — concurrent writers are serialized by git or Hub CAS like every other file. Also not a place for task *state* to accumulate on the spine: a task that grows design notes earns a ticket file and the line links to it, keeping the spine dense.

**Where:** `template/backlog/INDEX.md`; rule 13 of `template/AGENTS.md`; `agentsfs/rfcs/backlog-and-tasks.md` (the 0.10.0 design RFC, with the full worked example) and `agentsfs/rfcs/backlog-directories.md` (the 0.11.0 directory shape).
