...
For a focused Hub conversation outside review and scheduled maintenance, the hosted agent injects read-only orientation (bounded bootstrap plus recent unconsolidated episodes). Journals are created lazily only when there is learning, a decision, or useful context worth preserving. The stable key uses the durable conversation/thread and workspace, never the turn ID. Follow-up turns and workspace focus switches reuse the same entry. Expected content hashes protect checkpoints; ownership checks prevent editing another conversation’s journal. Turn completion and failure do not close or interrupt it. Explicit conversation closure can finish it; a final reply or voice disconnect cannot. Read-only and review scopes do not write journals.

Scheduled gardening receives the exact canonical `prompts/gardening.md` embedded in the deployed Hub with each authenticated maintenance job. The hosted app adds only execution adapters for its tools, permissions, and time/write limits; it has no separate gardening policy and does not load a replacement prompt from workspace content. Updating that canonical prompt and deploying the Hub updates future jobs. Scheduled gardening has dedicated prepare/consolidate tools. A maintenance grant may archive only unchanged complete or explicitly interrupted source episodes and publish the bounded bootstrap in the same Hub commit. It cannot start or rewrite an episode through that endpoint, or use archival as arbitrary deletion authority. The synthesis follows the bundled journal contract: adaptive historical resolution and links to authoritative knowledge notes. Computer-mode agents use the pinned CLI, `afs prime`, and the live journal contract; portable harnesses can follow the same file protocol without hooks. Application deployment does not overwrite customized workspace contracts.
