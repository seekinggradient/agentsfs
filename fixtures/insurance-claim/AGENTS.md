...
If the `afs` CLI is installed, `afs status` reports this instance's contract, git, and sync state; from a directory above several workspaces, `afs status <search-root>` discovers and reports all of them. Then `afs tree` prints this instance's whole tree with every description and freshness date. On a large instance, scope it: `afs tree <dir>` shows just that subtree and `--depth N` caps how deep it expands. Without the CLI, plain tools do the same jobs:
