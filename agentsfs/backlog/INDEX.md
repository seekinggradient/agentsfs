---
description: Prioritized backlog — pending work, discovered tasks, and parked ideas for this workspace.
...
## Next
- [ ] Release the tested same-workspace HTML image fix on branch `codex/hub-html-linked-images`; verify all ten SVG diagrams on the cloudwindow-terminal explainer after deployment. Relative img src assets now embed as bounded data images; file-scoped shares, CSS URLs, and srcset retain existing behavior ^hub-html-linked-images
...
- [ ] Report upstream that markdownto's guided-narration reader drops the whitespace at a soft line break when importing a `markdown` source, so a hard-wrapped article comes back with its wrapped words joined ("at the\nmoment" -> "at themoment"). Every `[target-quote::]` spanning a wrap then matches nothing, and the reader's source load is all-or-nothing: 19 of 54 beats fail on `cloudwindow-architecture.guided-narration.md`, and all 54 resolve when the same reader is handed that article's committed capture JSON instead. The Hub delivers byte-identical bytes either way and must not work around it — unwrapping would alter the file and deriving blocks in Go would mean owning a renderer. A re-vendor after the importer is fixed needs no Hub change ^guided-markdown-softbreak
- [ ] Add a read-only `tasks` tool to the local AgentsFS MCP server so MCP-only agents can list in-progress, ready, completed, and owner-blocked work across one or many workspaces. The CLI already supports this; the MCP server still exposes 12 tools and has no task-listing tool ^mcp-tasks-tool
...
- [ ] Extend automatic gardening from “scheduled doctor-driven edits” into a complete, governed fleet maintainer. Repository isolation, scoped grants, retries, per-repository progress, a six-successful-write/four-minute bound, and deterministic doctor-driven work already exist; journal consolidation now has scoped archive authority; the remaining capability is governed backlog ticket sweeping and broader archive/move/delete authority, with previews and an audit trail ^continual-fleet-gardener
...
- [ ] Add a Hub page that aggregates active backlog work across all of a user's workspaces. `afs tasks <search-root>` already provides the local CLI version, but the Hub has no account-level “all my Now items” view ^cross-kb-backlog
