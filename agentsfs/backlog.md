...
- [ ] Decide the content domain for `/render` sandboxed HTML (open since the share-links ship; now also a prerequisite consideration for [[#^markdownto-render]]) ^render-content-domain — SCOPE NARROWED 2026-08-09: the markdownto integration no longer waits on this; bundle-rendered board frames use opaque-origin srcdoc (playground model). This item now covers only arbitrary user-authored HTML served via /render
