...
- `garden_doctor` found 26 session notes pending consolidation and two closed ticket files still under `backlog/`: [[backlog/contract-backlog-spec]] and [[backlog/hub-projection-pull]].
...
- Reviewed the journal index, the five latest maintenance notes, and the production projection-sync record. The sampled operational facts are already represented in [[backlog/hub-projection-pull]]; no additional unique durable fact was safely foldable without a broad review.
