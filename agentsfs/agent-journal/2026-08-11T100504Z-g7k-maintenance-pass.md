...
- The production projection-sync record contained one durable operational detail not present in the ticket: the byte-identical dirty state of `agentic-stocks` was retained in a named stash during reconciliation rather than discarded. That detail was folded into [[backlog/hub-projection-pull]].
...
## Written directly
- Updated [[backlog/hub-projection-pull]] with the retained `agentic-stocks` stash detail and its source path.
