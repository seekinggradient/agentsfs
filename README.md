...
Layer 1: AgentsFS adds prompts and a canonical file format. Agents are encouraged to retain knowledge by writing, recall knowledge by reading, and periodically clean up what they have saved. Markdown notes carry one-line `description:` frontmatter, use `[[wikilinks]]`, cite sources, and live in directories that explain themselves with `INDEX.md`. A reserved session journal (a directory marked `agentsfs_role: journal`, `agent-journal/` by default) preserves learning and decisions in one entry per entire conversation, reused across turns; gardening folds completed episodes into a bounded bootstrap chronology and durable notes, then retains the original episodes in an archive. An existing vault or notes folder adopts in place by declaring its diaries, daily notes, and media folders as collections (`agentsfs_role: collection`) — described collectively by their `INDEX.md`, so the per-file rules don't apply beneath them and nothing has to be rewritten. This makes knowledge easy for agents to parse and easy for humans to inspect. It also works well in existing tools like Obsidian because it is just files, YAML frontmatter, and wiki-style links.

Layer 2: AgentsFS adds an agent-friendly capability layer. The `afs` CLI and MCP server discover and summarize every local instance with `afs status`, provide progressive disclosure with `afs tree`, health checks with `afs doctor`, search, backlinks, link-aware rename, reindexing, and more. `afs roles` reports where the reserved roles actually live, so a tool built on AgentsFS can ask instead of hardcoding directory names the contract owns and has changed before. These tools augment the agent's existing filesystem abilities; they do not replace the filesystem.
...
afs tasks        the backlog's derived view: in-progress work, questions waiting on you, ready work by band, blocked/parked/done counts; point it at a folder of instances for cross-project triage
afs task         claim or close one task by slug — flips the marker, logs to the ticket, commits, and pushes
...
`afs journal` manages episodes and consolidation; `afs docs journal` teaches the lifecycle from [template/agent-journal/INDEX.md](template/agent-journal/INDEX.md).
