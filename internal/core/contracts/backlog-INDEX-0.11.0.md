...
> Name a task with a trailing `^kebab-slug` and reference it as `[[#^slug]]` here or `[[backlog/INDEX#^slug]]` from anywhere. Block with `— blocked by [[#^slug]]` (lifts when every named task closes; plain-text blockers hold until edited away).
