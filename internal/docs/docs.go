...
var commands = []Command{
	...
	{"Orient", "afs prime [path] [--budget N]", "the session orientation pack: identity, in-progress and ready tasks, an adaptive tree, the two newest journal entries, and where to go next — assembled to a token budget (default 4000, chars/4)"},
	{"Orient", "afs tree [dir] [-d|--depth N] [--budget N]", "the tree with descriptions and freshness; scope to dir, cap depth, or pass --budget N to get the richest view that fits N estimated tokens (an explicit --depth wins over --budget)"},
	{"Orient", "afs tasks [path|search-root] [--all|--ready|--band NAME|--blocked-on-owner|--done] [--all-instances] [--json]", "derived view of the backlog: in-progress work, questions parked on the owner, then ready work by band, then blocked/parked/done counts; --done reads the archive, and a search root (or --all-instances) groups the same view per instance"},
	{"Orient", "afs task claim <slug> | done <slug> [--drop] [path] [--dry-run]", "flip one task's checkbox ([ ]→[/], or →[x]/[-]), log it on the ticket file when there is one, then commit and push"},
	...
	{"Orient", "afs roles [path] [--json]", "where the reserved roles live (journal, scratch, collections, backlog) — ask instead of hardcoding names"},
