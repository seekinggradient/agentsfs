...
Commit `package-lock.json`, source files, and the generated `../assets/editor.js` and `editor.js.LEGAL.txt` together. CSS lives in `../assets/editor.css`. The build is deterministic and dependency versions are pinned. The browser tests use installed Google Chrome, launch an opt-in Go fixture on `127.0.0.1:3347`, and exercise real commits in temporary repositories. The fixture is test code only. For a local interactive preview:
